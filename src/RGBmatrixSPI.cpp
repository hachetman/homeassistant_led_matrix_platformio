...
void RGBmatrixSPI::scroll( uint32_t scroll) {
  ...
  for (int row = 0; row < matrix_width; row++) {
    for (int column = 0; column < matrix_height; column++) {
      if (row < 20) {
        spi.transfer(*(transferBuffer+ ((row * matrix_height + column) << 1)));
        spi.transfer(*(transferBuffer+ ((row * matrix_height + column) << 1) + 1));
      } else {
        if (column < scroll) {
          // new data
          spi.transfer(*(transferBuffer+ ((row * matrix_height + column + 64 - scroll) << 1)));
          spi.transfer(*(transferBuffer+ ((row * matrix_height + column + 64 - scroll) << 1) + 1));
        } else {
          spi.transfer(*(transferBuffer+ ((matrix_height*(matrix_width-1) +
                                          row * matrix_height + column + 64 - scroll) << 1)));
          spi.transfer(*(transferBuffer+ ((matrix_height*(matrix_width-1) +
                                          row * matrix_height + column + 64 - scroll) << 1) + 1));
        }
      }
    }
  }
